...
# @Last Modified by:
# @Last Modified time: 2022-03-25 16:34:11
...
# # --------------------------------------------------------------------------
#  ggdensity.bg_num + ggdensity.chklst_prov_num + ggdensity.new_sum.L3.code + ggdensity.residence_time_to_2020 + ggdensity.Height_max_max + ggdensity.hab_suit_prop_max
...
# # --------------------------------------------------------------------------
# ggqqplot.bg_num + ggqqplot.chklst_prov_num + ggqqplot.new_sum.L3.code + ggqqplot.residence_time_to_2020 + ggqqplot.Height_max_max + ggqqplot.hab_suit_prop_max
...
checklist_Invspc_new$hab_suit_prop_max      <- scale(log(checklist_Invspc_new$hab_suit_prop_max + 0.001))
...
ggdensity(checklist_Invspc_new$hab_suit_prop_max, main = "Density plot", xlab = "hab_suit_prop_max")
...
# --------------------------------------------------------------------------
# table 1
# climatic suitability, hab_suit_prop_max (variable name in glm)
# --------------------------------------------------------------------------
# check the replicates and data range of hab_suit_prop_max
table(checklist_Invspc_new$naturalized_status[!is.na(checklist_Invspc_new$hab_suit_prop_max)])

# model fitting
model_hab_suit <- glm(naturalized_status ~ hab_suit_prop_max,
                      family = binomial(link = "cloglog"),
                      data = checklist_Invspc_new[!is.na(checklist_Invspc_new$hab_suit_prop_max),])
...
write.csv(nag.R2_summary, "D:/R_codes_for_nat.pla_in.China/Results/Table.1_nag.R2_summary.csv")
...
write.csv(summary_output.new, "D:/R_codes_for_nat.pla_in.China/Results/Table.1.and.Appendix.table.2_without_intercept.csv")
...
table(checklist_Invspc_new$naturalized_status[!is.na(checklist_Invspc_new$hab_suit_prop_max)])
range(checklist_Invspc_new$hab_suit_prop_max[!is.na(checklist_Invspc_new$hab_suit_prop_max)])

# plotting
plot_hab_suit_01 <- ggplot(data = checklist_Invspc_new[!is.na(checklist_Invspc_new$hab_suit_prop_max), ],
                           aes(x = hab_suit_prop_max, y = naturalized_status)) +
                    geom_rangeframe(data = data.frame(hab_suit_prop_max = c(-2, 2.2),
                                                      naturalized_status = c(0, 1)),
                                                      size = 1) +
                    theme(panel.background = element_rect(fill = NA),
                          # axis.line = element_line(size = 1, linetype = "solid"),
                          ...
                          axis.title = element_text(family = "serif", colour = "black", size = 14)) +
                    scale_y_continuous(limits = c(-0.025, 1.025), breaks = seq(0, 1, by = 0.2)) +
                    scale_x_discrete(limits = seq(-2, 2.2, by = 1)) +
...
# create prediction + confidence intervals, using ciTools package
model_hab_suit_var  <- data.frame(hab_suit_prop_max = seq(-2, 2.2, 4.2/1000))
...
plot_hab_suit_02 <- plot_hab_suit_01 +
                    ...
                    geom_line(data = model_hab_suit_pred, aes(x = hab_suit_prop_max, y = pred), size = 1, colour = "black") +
                    geom_line(data = model_hab_suit_pred, aes(x = hab_suit_prop_max, y = LCB0.025), size = 1, colour = "blue", linetype = "dashed", alpha = 1) +
                    geom_line(data = model_hab_suit_pred, aes(x = hab_suit_prop_max, y = UCB0.975), size = 1, colour = "blue", linetype = "dashed", alpha = 1)
...
# export fig.2
ggexport(figure_012, filename = "D:/R_codes_for_nat.pla_in.China/Results/Figure.2.png",
                     width = 3600,
                     height = 4800,
                     pointsize = 12,
                     res = 300)