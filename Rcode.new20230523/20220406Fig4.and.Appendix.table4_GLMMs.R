...
# @Last Modified by:
# @Last Modified time: 2022-03-25 16:31:45
...
# hab_suit_prop_max.y: climatic suitability in province, 0-1
# ==========================================================================
# preparation work
# ==========================================================================
# cleaning memory
...
checklist_Invspc_new_prov <- read.csv("D:/R_codes_for_nat.pla_in.China/data/checklist_nat.pla_in_provinces.of.China.csv")
skim(checklist_Invspc_new_prov)
str(checklist_Invspc_new_prov)

# scale the data of hab_suit_prop_max.y after log-transformation
...
checklist_Invspc_new_prov$hab_suit_prop_max.y_scaled <- scale(log(checklist_Invspc_new_prov$hab_suit_prop_max.y + 0.001))
checklist_Invspc_new_prov$hab_suit_prop_max.y_scaled <- as.numeric(checklist_Invspc_new_prov$hab_suit_prop_max.y_scaled)

# check data range of transformed hab_suit_prop_max.y
range(checklist_Invspc_new_prov$hab_suit_prop_max.y_scaled)
...
checklist_Invspc_new_prov_nat <- checklist_Invspc_new_prov %>% filter(naturalized_status == 1)
...
# # --------------------------------------------------------------------------
# # model fitting
# m0 <- glmer(naturalized_status_prov ~ hab_suit_prop_max.y_scaled +
#             prov_planting_status +
#             hab_suit_prop_max.y_scaled:prov_planting_status +
...
model_interaction_forNat <- glmer(naturalized_status_prov ~ hab_suit_prop_max.y_scaled +
                                  prov_planting_status +
                                  hab_suit_prop_max.y_scaled:prov_planting_status +
                                  (1|Standardized_names) + (1|prov_code),
                                  family = binomial(link = "cloglog"),
...
# dispersion test
# the result of dispersion test is 0.846
...
write.csv(tidy.model_interaction_forNat, "D:/R_codes_for_nat.pla_in.China/Results/GLMMs_nat.status.in.prov.csv")
...
var.interaction_forNat <- with(checklist_Invspc_new_prov_nat,
                               expand.grid(hab_suit_prop_max.y_scaled = seq(min(hab_suit_prop_max.y_scaled), max(hab_suit_prop_max.y_scaled), length.out = 100),
...
write.csv(interaction_forNat_pred, "D:/R_codes_for_nat.pla_in.China/Results/Prediction_based.on.GLMMs.for.nat.status.in.prov.csv", row.names = FALSE)

# check range of prediction data
range(checklist_Invspc_new_prov_nat$hab_suit_prop_max.y_scaled)
...
plot_interaction_forNat_01 <- ggplot(data = checklist_Invspc_new_prov_nat,
                                     aes(x = hab_suit_prop_max.y_scaled,
                                         y = naturalized_status_prov),
                                     group = factor(prov_planting_status),
                                     colour = factor(prov_planting_status)) +
                              geom_rangeframe(data = data.frame(hab_suit_prop_max.y_scaled = c(-1, 2.5),
                              ...
                              scale_x_continuous(limits = c(-1, 2.5), breaks = seq(-1, 2.5, by = 0.5)) +
...
plot_interaction_forNat_02 <- plot_interaction_forNat_01 +
                              geom_jitter(aes(x = hab_suit_prop_max.y_scaled, y = jitter.position, colour = factor(prov_planting_status)), shape = 1, size = 2, stroke = 0.5, width = 0, height = 0.01, alpha = 0.5) +
                              geom_line(data = interaction_forNat_pred, aes(x = hab_suit_prop_max.y_scaled, y = pred, colour = factor(prov_planting_status)), size = 1) +
                              geom_line(data = interaction_forNat_pred, aes(x = hab_suit_prop_max.y_scaled, y = LCB0.025, colour = factor(prov_planting_status)), size = 1, linetype = "dashed", alpha = 1) +
                              geom_line(data = interaction_forNat_pred, aes(x = hab_suit_prop_max.y_scaled, y = UCB0.975, colour = factor(prov_planting_status)), size = 1, linetype = "dashed", alpha = 1) +
...
# export fig.4
ggexport(plot_interaction_forNat_02, filename = "D:/R_codes_for_nat.pla_in.China/Results/Figure.4.png",
    width = 1600,
    height = 1600,
    pointsize = 12,
    res = 300)